...
function! s:PHPCustomFolds() " {{{
	...
	call s:PHPFoldPureBlock('^\s*\(public \|protected \|private \)\?\(static \)\?function', s:FOLD_WITH_PHPDOC)
	call s:PHPFoldPureBlock('^\s*\(static \)\?\(public \|protected \|private \)\?function', s:FOLD_WITH_PHPDOC)
	call s:PHPFoldProperties('^\s*abstract \(public \|protected \|private \)\?function', ";", s:FOLD_WITH_PHPDOC, 1, 0)

	" Fold class properties with PhpDoc (var/public/private/protected $foo = NULL;)
	call s:PHPFoldProperties('^\s*var\s\$', ";", s:FOLD_WITH_PHPDOC, 1, 0)
	call s:PHPFoldProperties('^\s*public\s\(static\s\)\?\$', ";", s:FOLD_WITH_PHPDOC, 1, 0)
	call s:PHPFoldProperties('^\s*private\s\(static\s\)\?\$', ";", s:FOLD_WITH_PHPDOC, 1, 0)
	call s:PHPFoldProperties('^\s*protected\s\(static\s\)\?\$', ";", s:FOLD_WITH_PHPDOC, 1, 0)

	" Fold class contants with PhpDoc (const FOO = NULL;)
	call s:PHPFoldProperties('^\s*const\s[A-Z]', ";", s:FOLD_WITH_PHPDOC, 1, 0)
	...
	call s:PHPFoldProperties('^\s*Zend_Loader::loadClass(', ";", s:FOLD_WITH_PHPDOC)
